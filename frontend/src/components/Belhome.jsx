...
const Belhome = () => {
  ...
  const WishlistButton = ({ product }) => {
    ...
    return (
      <motion.button
        ...
        className={`flex items-center gap-2 px-3 py-2 rounded-2xl text-sm font-medium transition-shadow focus:outline-none ${isWishlisted ? "bg-red-500 text-white" : "bg-gradient-to-r from-cyan-400 to-indigo-500 text-gray-900 shadow-md"}`}
      >
        <FaHeart />
      </motion.button>
    );
  ...
  return (
    ...
    <div className="overflow-x-hidden">
      <div className="mt-6 px-4 sm:px-6 md:px-8 lg:px-10 max-w-7xl mx-auto w-full text-white box-border">
        <header className="text-center mb-4">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold">Summer Collection</h1>
          <p className="text-base sm:text-lg md:text-xl font-semibold mt-2 text-white/95">Clothes & Accessories</p>
        </header>
        ...
        <div className="flex flex-wrap justify-center gap-3 sm:gap-6 mb-6">
          {["All", "Hoodies", "T-Shirts", "Caps"].map((cat) => (
            <button
              ...
              className={`px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm md:text-sm font-medium transition-all ${filter === cat ? "bg-gradient-to-r from-cyan-400 to-indigo-500 text-gray-900 shadow-lg" : "bg-white/6 text-white/90 hover:bg-white/10"}`}
            >
              {cat}
            </button>
          ))}
          ...
          {filtered.map((costume) => {
            ...
            return (
              ...
              >
                ...
                <h2 className="mt-3 font-semibold text-white/100 text-sm sm:text-base">{costume.desc}</h2>
                <p className="text-cyan-300 font-bold mt-1 text-sm sm:text-base">${costume.price}</p>
        ...
        {fimage1.map((img, index) => (
          <div key={index} className="relative mt-10 rounded-2xl overflow-hidden w-full">
            ...
            <div className="absolute inset-0 flex flex-col justify-center items-start p-4 sm:p-6 md:p-12 bg-gradient-to-r from-black/35 to-black/10">
              <p className="text-sm md:text-lg font-semibold text-cyan-300">{img.desc}</p>
              <p className="text-base sm:text-lg md:text-2xl font-bold text-white mt-2">{img.titl}</p>
              <Link to="/" className="mt-4 inline-block px-3 py-2 rounded-2xl bg-gradient-to-r from-cyan-400 to-indigo-500 text-gray-900 font-semibold text-xs sm:text-sm">
                {img.btna}
              </Link>
            </div>
          </div>
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {imagefooter.map((item, index) => (
            <div key={index} className="glass-dark rounded-xl p-4 flex flex-col items-start">
              <h3 className="text-white/95 font-semibold text-base sm:text-lg md:text-xl">{item.heading}</h3>
              <button className="mt-2 text-cyan-300 text-sm sm:text-base">{item.btn}</button>
            </div>
          ))}
        </div>
            ...
            {fimage.map((img, index) => (
              <div key={index} className="relative rounded-2xl overflow-hidden w-full">
                ...
                <div className="absolute inset-0 flex flex-col justify-center items-start p-4 sm:p-6 md:p-12 bg-gradient-to-r from-black/35 to-transparent">
                  <p className="text-cyan-300 font-semibold text-sm sm:text-base md:text-lg">{img.desc}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-8">
            {imagefooter2.map((item, index) => (
              <div key={index} className="glass-dark rounded-xl p-4 flex gap-4 items-center">
                <p className="text-cyan-300 text-2xl">{item.icon}</p>
                <div>
