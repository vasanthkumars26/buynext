    ...
    return (
        <div className='mt-5 text-lg'>
            ...
            <div className='grid grid-cols-2 md:grid-cols-4 mt-4 text-start  gap-x-4 gap-y-2'>
                ...
                <button onClick={()=>addtoCart(costume)} className='flex items-center gap-1 border border-green-600 p-2 rounded-3xl hover:bg-green-300 hover:transition-transform transition-all'>Add to Cart<FaShoppingCart/> </button></div>
                </div>
    ))}
            </div>
            ...
            {fimage1.map((img)=>(
                ...
                <div className='absolute top-10 md:top-24 md:left-72 lg:top-44 lg:left-96 ml-52'> <p className='text-sm md:text-5xl font-bold text-black'>{img.desc}</p>
                    <p className='text-sm md:text-2xl mt-3 font-semibold mb-6'>{img.titl}</p>
                    <Link to="/" className='text-sm md:text-xl bg-green-950 p-2 rounded-2xl text-white hover:from-black hover:to-green-800' >{img.btna}</Link></div>
                </div>
            ))}
            <div className='grid-cols-1 md:flex justify-center md:grid-cols-3 bg-gradient-to-r from-green-900 to-purple-900 gap-10 py-1 shadow-xl shadow-gray-400'>
            {
                imagefooter.map((item)=>(
                        ...
                        <div className='border-b border-b-gray-400 md:border-r border-r-gray-400' >
            ...
            <div className='mt-10'>
                ...
                <div className='flex gap-4 text-sm mt-2'>
                <WishlistButton product={costume} />
                <button onClick={()=>addtoCart(costume)} className='flex items-center gap-1 border border-green-600 p-2 rounded-3xl hover:bg-green-300 hover:transition-transform transition-all'>{costume.btn2}<FaShoppingCart/> </button></div>
                </div>
    ))}</div>
                <div className='grid grid-cols-2 md:grid-cols-4 mt-4 text-start  gap-x-4 gap-y-2'>
                {pow.map((costume)=>(
                ...
                <h1 className='mt-2'>{costume.desc}</h1>
                <p className='text-green-600 font-semibold'>${costume.price}</p>
                <div className='flex gap-4 text-sm mt-2'><button className='flex items-center gap-1 bg-gradient-to-r from-green-300 to-green-600 p-2 rounded-2xl'>{costume.btn1}<FaHeart/></button>
                <button className='flex items-center gap-1 border border-green-600 p-2 rounded-3xl hover:bg-green-300 hover:transition-transform transition-all'>{costume.btn2}<FaShoppingCart/> </button></div>
                </div>
    ))}</div>
    <div className='grid grid-cols-2'>
                {fimage.map((img)=>(
                <div className=' relative mt-10 w-[100%] text-white'>
                    <img src={img.img} alt="fimage" />
                <div className='absolute w-40 md:w-[60%] top-1 right-20  md:top-24 md:left-72 lg:top-34 lg:left-0 ml-52'> <p className='  text-sm md:text-5xl font-bold'>{img.desc}</p>
                    <p className='hidden  md:block text-sm md:text-2xl mt-3 font-semibold mb-6'>{img.titl}</p>
                    <Link to="/" className='text-sm md:text-xl bg-green-950 p-2 rounded-2xl mt-7 text-white hover:from-black hover:to-green-800' >{img.btna}</Link></div>
                </div>
            ))}</div>
            ...
            {
                imagefooter2.map((item)=>(
                        <div className='border-b flex gap-5 items-center justify-center border-b-gray-400 md:border-r border-r-gray-400' >
                        <p>{item.icon}</p>
                        <div><h1 className='text-gray-300 font-semibold md:text-xl p-2 '>{item.heading}</h1>
                        <p className='text-sm md:text-lg text-gray-300' >{item.subhead}</p></div> </div>
